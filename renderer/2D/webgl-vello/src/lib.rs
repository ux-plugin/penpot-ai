...
// The C-style ABI + host scene-state singleton now live in render_core::vello so the classic backend
...
// The generic GPU sink also lives in render_core::vello now (it needs only the shared abi + effect

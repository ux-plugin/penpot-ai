...
pub mod bits {
    ...
    /// above it — into its output record, resampled by record 0's scale (a box average; 1 is a
    /// copy). The tile's own pixels are left as they are.
    ...
    /// Sampling head: resample the value between resolutions (by record 0's scale: a box average
    /// when whole, bilinear otherwise; `u[0].y` = what a read past the value yields, one of
    /// [`RESAMPLE_CLAMP`], [`RESAMPLE_TRANSPARENT`], [`RESAMPLE_KEEP`]).
...
/// record 3 = the FIELD's distance input, record 4 = the arm's OUTPUT, record 5 = the field anchor
/// then the scales of records 0–3 (see [`REC_SCALES`]). Source 0 = absent, 1 = a store rect, 2 =
/// the tile's own registers (the state it holds), 3 = the marker's own silhouette.
pub const REC_STRIDE: usize = 8;

/// Where record 5 holds the read operands' scales, past the 26-float header: record `i`'s (0–3)
/// at `off + 26 + REC_SCALES + i` — the texels of the value it reads per texel of the arm's own
/// resolution, so every input is read at its own resolution; 1 when it is the arm's, or absent.
pub const REC_SCALES: usize = 5 * REC_STRIDE + 4;
...
pub fn stamp_field_anchor(desc: &[f32; 26], rec: &mut [[f32; 4]; 12]) {

...
impl Sink {
    ...
    pub fn render_whole_viewport<B: RasterBackend>(
        ...
        let mut root_of: HashMap<u128, usize> = HashMap::new();
        let served: Vec<(u128, usize)> = if std::env::var("WV_DEMAND").is_ok_and(|v| v == "0") {
            ...
            fn tail_root(
                ...
                use crate::vello::frame_dag::Source as DagSource;
                use crate::vello::units::{RasterSource, UnitOp};
                dag.nodes[n]
                    .inputs
                    .iter()
                    .filter_map(|&t| {
                        ...
                        tail_root(dag, t, shape).map(|(p, r)| (p + f64::from(dag.nodes[t].pad), r))
                    })
                    .fold(None, |a: Option<(f64, usize)>, p| {
            ...
            for c in 0..dag.nodes.len() {
                ...
                let Some((p, root)) = tail_root(&dag, c, shape) else { continue };
                let seed = dag.nodes[c].reach.map_or(frame_rect, |r| r.intersect(frame_rect));
                if seed.width() <= 0.0 || seed.height() <= 0.0 {
                    continue;
                }
                // A shape can carry several chains (a shadow AND a layer blur); the backdrop
                // root wins the region's lowering when both kinds serve.
                match root_of.get(&shape) {
                    Some(&r0)
                        if matches!(dag.nodes[r0].op, crate::vello::units::UnitOp::Reload) => {}
                    _ => {
                        root_of.insert(shape, root);
                    }
                }
        ...
        struct BandMark {
            node: usize,
            rect: [u32; 4],
            /// The mark's baked arm — `None` for a fence (zero desc).
            desc: Option<[f32; 26]>,
            masked: bool,
            /// Serve-test inflation for the mark's route (kernel or displacement bound).
            slack: f32,
            /// A fence's window opens on the page colour (a backdrop ground/window) or on
        ...
        for &(reader, ri) in &served {
            ...
            if let Some(root) = body_root {
                ...
                for c in 0..n_before {
                    if dag.nodes[c].inputs.contains(&root)
                        && matches!(dag.nodes[c].source, crate::vello::frame_dag::Source::Effect { shape, .. } if shape == reader)
                    {
                        dag.nodes[c].inputs.push(node);
                    }
                }
            ...
            let value_node = if let Some((vals, chains_l)) = fold_leases {
                ...
                for (i, &(_, _, h, _)) in writers.iter().enumerate() {
                    ...
                    band_plan.push((reader, BandMark {
                        ...
                        desc: Some(crate::vello::bake::blur_arm(
                            sigma * k,
                            linear,
            ...
            if body_root.is_none() {
                ...
                for i in 0..orig_len {
                    ...
                    {
                        band_of.insert(i, (value_node, ri, false));
                    }
                }
            }
            // The reader's own chain, generalized: every SPILLED, GATHER-TAPPED node of the chain
            // gets a band instance over its input's lease — a blur mirrors at the region density,
            // a lens head re-anchors its field uniforms to the lease's grid frame (same
            // `lens_device_field`, region origin + k) — and the read edge into each original
            // consumer orders the schedule. Atlas sides alternate off the source's side so a
            // dispatch never reads the texture it writes; `band_of` hands the reader's route
            // stamping the finished lease per original node.
            let lens = crate::vello::abi::with_scene(|live, _, modifiers| {
                let n = live.get(reader)?;
                let m = modifiers.get(&reader).copied().unwrap_or(Affine::IDENTITY);
                crate::effect_graph::lens_geometry(n, m)
            });
            for j in 0..orig_len {
                if !matches!(dag.nodes[j].source, crate::vello::frame_dag::Source::Effect { shape, .. } if shape == reader)
                ...
                let Some(&(src_node, src_lease, src_chain)) =
                    dag.nodes[j].inputs.first().and_then(|i| band_of.get(i))
                else {
                    continue;
                };
                use crate::vello::units::UnitOp;
                let (op_inst, desc, slack, ctl) = match &dag.nodes[j].op {
                    UnitOp::Blur { sigma, linear, axis, edge } => {
                        ...
                        (
                            UnitOp::Blur { sigma: sigma * k, linear, axis, edge },
                            crate::vello::bake::blur_arm(
                                sigma * k,
                                linear,
                                axis == crate::vello::units::BlurAxis::Y,
                                crate::vello::bake::Policy::default(),
                    ...
                    | UnitOp::MaskMix(u0) => {
                        ...
                        let origin = (
                            ...
                            r.source[1] - f64::from(r.grid[1]) / r.k,
                        );
                        let mut u = crate::effect_graph::lens_device_field(
                            g, *geom, (bw, bh), origin, full_view, r.k,
                        )
                        .to_vec();
                        for slot in 17..21 {
                            if let Some(&v) = u0.get(slot) {
                                u[slot] = v;
                            }
                        }
                        let op = match &dag.nodes[j].op {
                            UnitOp::Warp(_) => UnitOp::Warp(u),
                            UnitOp::Scatter(_) => UnitOp::Scatter(u),
                            UnitOp::Shade(_) => UnitOp::Shade(u),
                            _ => UnitOp::MaskMix(u),
                        };
                ...
                let inst = dag.push_region_blur(
                    lease,
                    op_inst,
                    grid_rect(&regions, lease),
                    vec![src_node],
                    format!("region {ri} step of {j}"),
                );
                if !src_chain {
                    writes_chain.insert(inst);
                }
                band_plan.push((reader, BandMark {
                    node: inst,
                    rect: regions.regions[lease].grid,
                    desc: Some(desc),
                    masked: false,
                    slack,
                    ctl,
        ...
        if std::env::var("WV_DBG_REGIONS").is_ok() {
            ...
            for &(reader, ref bm) in &band_plan {
                eprintln!(
                    "WV_DBG_BANDS: reader={:04x} node={} rect={:?} arm={} masked={} mask={:?} route={:?} draws={:?}",
                    (reader & 0xffff) as u16,
                    bm.node, bm.rect,
                    bm.desc.map_or(0, |d| d[0] as u32),
                    bm.masked,
        ...
        let sil_fold = true;
        {
            ...
            // writer's silhouette in region space, and carries its follow-on draws in `after`.
            {
                ...
                for &(reader, ref bm) in &band_plan {
                    ...
                    let desc = bm.desc.unwrap_or([0.0f32; 26]);
                    if bm.desc.is_some() {
                        crate::vello::bake::stamp_field_anchor(&desc, &mut rec);
                    }
                    let masked = bm.masked;
            ...
            for (&gid, ms) in marks.iter_mut() {
                for m in ms.iter_mut() {
                    ...
                    if let Some(&ri) = region_of_reader.get(&gid) {
                        ...
                        let (out, chain) = banded.unwrap_or((ri, false));
        ...
        let region_draws: HashMap<usize, (usize, usize, usize, bool)> = band_plan
            .iter()
            .filter_map(|&(_, ref bm)| {
                bm.desc
                    .is_none()
                    .then(|| bm.draws.map(|(t, lo, hi)| (bm.node, (t, lo, hi, bm.seed))))
                    .flatten()
            })
            .collect();
        ...
        let (boundaries, markers_before, total_markers): (Vec<u32>, Vec<u32>, u32) = {
            ...
            for (j, &(gi, gid, kind)) in gathers.iter().enumerate() {
                if gi > cursor {
                    draw_frame_range(backend, &mut scene, root, cursor, gi);
                    cursor = gi;
                }
                    ...
                    if let Some(ms) = marks.get(&gid) {
                        for m in ms {
                                ...
                                (m.desc[0] == 0.0 && !is_fence(m), &dag.nodes[m.node].op)
                            {
                                ...
                                let t = root * Affine::translate((f64::from(offset[0]), f64::from(offset[1])));
                                draw_frame_range(backend, &mut scene, t, gi, gi + 1);
                                continue;
                            }
                            z += 1;
                            let eid = if m.masked {
            ...
            draw_frame_range(backend, &mut scene, root, cursor, usize::MAX);
            (b, mb, z)
        };
                ...
                if let Some(unit_nodes) = round_nodes.get(&window_lo) {
                    ...
                    if shp.to_draft {
                        let dv = if shp.region_out {
                            ...
                            // draws; every other band node runs its arm with taps resolving
                            // through its region route, writing whichever atlas the PLAN put its
                            // lease in (`writes_chain` — the functor alternates atlases so a
                            // dispatch never reads the texture it writes).
                            let dv = region_atlas
                                .clone()
                            ...
                            if shp.base == Slot::None {
                                ...
                                dv
                            } else if writes_chain.contains(&rep) {
                                let cv = region_chain
                                    .clone()
        ...
        if let Ok(dir) = std::env::var("WV_DUMP_ATLAS") {
            let dump = |tex: &wgpu::Texture, name: &str| {
                ...
                let path = format!("{dir}/{f:03}-{name}-{w}x{h}.raw");

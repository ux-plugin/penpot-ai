...
pub enum PaintOp {
    ...
    PushLayer(u128),
    /// Open a **mask** layer — a `DstIn`-composited layer whose one drawn body (the masked group's
    /// mask child, emitted as the following `Body`) multiplies the alpha of the content already
    /// painted into the enclosing isolation layer. Paired with a later `PopLayer`. This is the
    /// scheduler's half of a masked group: the group's isolation layer (a plain `PushLayer`) brackets
    /// the content, and this `DstIn` sub-layer clips it to the mask's silhouette/alpha — matching
    /// render-wasm's two-pass `SaveLayer(DstIn)` construction, so a soft (gradient/image/partial)
    /// mask masks softly, not just as a hard clip.
    PushMaskLayer,
...
pub enum Step {
    ...
    ComposeBackdrop {
        ...
        always_cap: bool,
        /// Declared quality floor `k ∈ (0, 1]` for the effect on this shape — the *free* downscale it
        /// tolerates (glass/custom `acceptable_downscale`; `1.0` for a background blur or an undeclared effect).
        /// The sink renders the backdrop (and downstream passes) at `min(resolution_cap, acceptable_downscale)`
        /// and upscales at the stamp. Independent of the reach-driven memory cap.
        acceptable_downscale: f32,
        /// How the backdrop fills the area past a **scoped** gather's content (see [`crate::model::TileMode`]).
        /// Sets the clear colour the scope tiles are composited over: `Decal` transparent, `Black` opaque
        /// black, `Clamp` extends the scope edge. Ignored for a top-level (page-scoped) gather.
        tile_mode: crate::model::TileMode,
        write_to: SurfaceRef,
    },
    ...
    /// Draw node `shape`'s `shadow`-th drop shadow as a **blurred silhouette**, composited into
    /// `write_to` behind the shape's own body. The sink renders the offset silhouette (in the shadow's
    /// colour) sharp into a temporary surface sized to `extent`, blurs it with the `run_graph` Gaussian
    /// at `sigma`, then composites the result into `write_to`. This is how a non-box shape (an
    /// arbitrary path) gets a soft drop shadow that follows its true outline — classic vello has no
    /// inline arbitrary-silhouette blur, so the blur must happen at the sink. `clip` is the tile's
    /// page-space rect. Box shapes keep the cheaper inline native blurred-rounded-rect instead.
    PaintPathShadow {
        shape: u128,
        shadow: usize,
        sigma: f32,

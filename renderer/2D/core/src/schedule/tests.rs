...
fn a_masked_group_brackets_its_mask_in_a_dstin_sublayer_after_the_content() {
    ...
    let content = rect(3, 100.0, 100.0, 300.0, 300.0);
    ...
    let step = sched
        .steps
        .iter()
        .find(|s| matches!(s, Step::Paint { ops, .. } if ops.contains(&PaintOp::PushMaskLayer)))
        .expect("a masked group emits a DstIn mask sublayer");
    let Step::Paint { ops, .. } = step else { unreachable!() };
    let pos = |want: &PaintOp| ops.iter().position(|o| o == want);
    let push_outer = pos(&PaintOp::PushLayer(1)).expect("masked group isolates even at opacity 1");
    let content_body = pos(&PaintOp::Body(3)).expect("content paints");
    let push_mask = pos(&PaintOp::PushMaskLayer).unwrap();
    let mask_body = pos(&PaintOp::Body(2)).expect("mask is drawn");

    assert!(push_outer < content_body, "isolation opens before content");
    assert!(content_body < push_mask, "content paints before the mask sublayer opens");
...
fn custom(reads_backdrop: bool, reach: f32) -> crate::model::CustomShader {
    crate::model::CustomShader { wgsl: String::new(), reach, param_vec4s: 0, params: vec![], reads_backdrop, acceptable_downscale: 1.0 }
}

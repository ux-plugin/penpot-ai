...
**Status: LIVE (2026-08-31; record 4 added 2026-09-02; record 5 + stride 8 on 2026-09-09).** The
migration below is complete: every mark packs 26 header floats plus 6 records
(`REC_COUNT`/`REC_STRIDE` in bake.rs), `fx_load_desc` is the single parser building the whole arm
(header + records), and header bits 15+ are free. The battery was byte-identical across all 24
scenes on the switch, and again on the stride widening.

Record 5 is the OVERFLOW role: where the value input's out-of-frame taps resolve. Its source code
`SRC_REGION` (4) names the region atlas — the lease store rented below the frame — and it is the
first source kind to use the widened stride: base row `[source, offset x, offset y, k]` (the
affine `atlas = pos * k + offset`) plus extension row `[lo x, lo y, hi x, hi y]` (the lease's own
texel rect, hi inclusive-minus-one, clamping a straying tap to lease edge-extend). A zero record
means no serving — escaped taps keep the frame edge-extend clamp. This row replaced the pre-ABI
rec5/rec6 region bolt-on (device source rect + sign-selected dual atlas).
...
- **Uniform stride.** Every record is 8 floats — a base row `[source, window x, window y, param]`
  plus an extension row for source kinds that need more than four facts (today only the OVERFLOW
  record's lease clamp rect; don't-care zeros elsewhere). A wider source kind widens the stride
  constant for everyone; records never self-describe their size.

## Record fields

| field | meaning | generated source (code 0) | sampled source (code ≥ 1) |
|---|---|---|---|
| `source` | which kind of place feeds this input: `0` generated by the arm's field math, `1` accumulator/backdrop, `2` strip, `3` draft, `4` region atlas | `0` | register id |

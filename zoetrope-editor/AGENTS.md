# Agent guide for `zoetrope-editor`

TypeScript package: Skia WASM renderer wrapper, React canvas UI, web worker spatial index, and shared `@zoetrope-editor/common/*` utilities.

## Commands

```bash
# From repo root (preferred)
pnpm --filter zoetrope-editor run dev          # Vite dev server on :5173
pnpm --filter zoetrope-editor run build        # Build lib + worker (two Vite configs)
pnpm --filter zoetrope-editor run build:lib    # Library only → dist/renderer.es.js, dist/renderer.cjs
pnpm --filter zoetrope-editor run build:worker # Worker only → dist/worker.js
pnpm --filter zoetrope-editor run test         # Vitest (node environment)
pnpm --filter zoetrope-editor run test:watch   # Vitest watch mode
pnpm --filter zoetrope-editor run lint         # ESLint
```
...
- **TypeScript**: `tsconfig.lib.json` (strict, ES2022, ESNext modules). Path alias `@/*` → `src/*`, `@zoetrope-editor/common/*` → `src/lib/common/*`.
...
`src/index.ts` is the **only** curated export surface. Consumers import from the `zoetrope-editor` package root. Do not add barrel `index.ts` files elsewhere — import from the defining file (e.g. `@zoetrope-editor/common/conversions`, not `@zoetrope-editor/common`).

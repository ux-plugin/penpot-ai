/**
 * Canonical shape ids for zoetrope-editor: Penpot v8 UUIDs (see uuid-impl `v8`).
...
export function assertValidAddObjChange(change: AddObjChange): void {
  ...
  if (!isCanonicalShapeId(id)) {
    throw new Error(
      `[zoetrope-editor] add-obj id must be a canonical 36-char UUID (Penpot v8). Got length=${id?.length ?? 0}: ${String(id).slice(0, 80)}`
    )
  }
  const oid = (change.obj as { id?: string }).id
  if (oid != null && oid !== id) {
    throw new Error(
      `[zoetrope-editor] add-obj change.id and obj.id must match. change.id=${id} obj.id=${oid}`
    )
  }
}

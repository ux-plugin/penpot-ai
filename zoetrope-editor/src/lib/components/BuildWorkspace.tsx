 ...
 *
 *   left   — component tree (live) + data stores + chat, a resizable vertical split
  ...
  return (
    ...
    >
      <ResizablePanelGroup orientation="horizontal" className="h-full w-full p-3" style={{ paddingRight: RIGHT_RAIL }}>
        {/* Left rail: components (top), data stores, chat/agent (bottom), each resizable. */}
        <ResizablePanel id="build-rail" minSize={14} defaultSize={26} className="min-h-0 min-w-0">
          <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-border/80 bg-white shadow-md">
            <ResizablePanelGroup orientation="vertical" className="h-full w-full">
              <ResizablePanel id="build-components" minSize={10} defaultSize={30} className="min-h-0 overflow-auto">
                <ComponentTree />
              </ResizablePanel>
              <ResizableHandle withHandle />
              <ResizablePanel id="build-stores" minSize={10} defaultSize={20} className="min-h-0 overflow-auto">
                <StoresPanel />
              </ResizablePanel>
              <ResizableHandle withHandle />
              <ResizablePanel id="build-chat" minSize={20} defaultSize={50} className="min-h-0 overflow-hidden">
                <ChatPanel />
              </ResizablePanel>
            </ResizablePanelGroup>
          </div>

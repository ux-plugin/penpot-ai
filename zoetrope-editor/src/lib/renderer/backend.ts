...
/**
 * `?renderer=` override: `vello` (hybrid, WebGL2) / `vello-gpu` (classic, WebGPU) / `auto`
 * (route by capability). `null` for neither (and under SSR). `auto` is not a backend artifact —
 * [`chooseBackendKind`] resolves it to a concrete one. Skia has been detached from the editor, so
 * `?renderer=skia` is no longer honored and falls through to the capability-routed default.
 */
type RendererOverride = BackendKind | 'auto'
function rendererOverride(): RendererOverride | null {
  if (typeof window === 'undefined') return null
  const value = new URLSearchParams(window.location.search).get('renderer')
  return value === 'vello' || value === 'vello-gpu' || value === 'auto'
    ? value
    : null
}
...
/**
 * Whether this browser can give us a WebGL2 context — the hybrid vello backend's target (it no
 * longer uses WebGPU; classic owns that). Near-universal, but probe rather than assume so an
 * explicit request degrades to Skia instead of failing inside wgpu's GL bring-up.
 */
...
/**
 * Route by device capability — the dual-vello dispatch. WebGPU → classic vello (compute, max GPU);
 * otherwise hybrid vello (WebGL2, the fallback). Skia has been removed as a backend, so classic
 * vello is now the default for every capable browser and hybrid vello is the floor. Shared by
 * `?renderer=auto` and the no-override default path.
 */
async function chooseByCapability(): Promise<BackendKind> {
 ...
 * anything is fetched. The default is classic vello (WebGPU compute); browsers without a WebGPU
 * adapter get hybrid vello (WebGL2). An explicit `?renderer=vello-gpu` on a browser without WebGPU
 * falls back to hybrid vello with a warning rather than failing deep inside wgpu bring-up.
 */
export async function chooseBackendKind(): Promise<BackendKind> {
  const override = rendererOverride()
  // Classic vello is WebGPU-only (compute rasterization); an absent adapter drops to hybrid vello
  // (WebGL2) rather than failing inside wgpu bring-up.
  if (override === 'vello-gpu') {
    if (await probeWebGPU()) return 'vello-gpu'
    console.warn('[renderer] ?renderer=vello-gpu requested but WebGPU is unavailable; using hybrid vello (WebGL2).')
    return 'vello'
  }
  // Hybrid vello is WebGL2-only — classic owns WebGPU.
  if (override === 'vello') return 'vello'
  // `auto` and the no-override default both route by capability, classic vello first.
  return chooseByCapability()
}

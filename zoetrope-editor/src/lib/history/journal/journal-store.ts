...
export interface JournalState {
   ...
   * commits is a leak worth warning about; the idle-coalescing path uses the
   * same timer as its normal commit mechanism, and warning there reports
   * healthy behaviour as a fault.
   */
  begin: (id: string, timeoutMs?: number, onTimeout?: 'warn' | 'expected') => void
...
export const useJournalStore = create<JournalState>()((set, get) => ({
  ...
  begin: (id, timeoutMs = TRANSACTION_TIMEOUT_MS, onTimeout = 'warn') => {
    set((s) => ({
...
export function markJournalInteraction(id = 'interaction', idleMs = INTERACTION_IDLE_MS): void {
  ...
  useJournalStore.getState().begin(id, idleMs, 'expected')
}

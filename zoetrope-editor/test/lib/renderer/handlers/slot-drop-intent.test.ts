...
describe('slot drop intent', () => {
  ...
  it('arms when a single frame is dragged with the cursor over a slot', () => {
    const intent = resolveSlotDropIntent(new Set(['home']), PAGE_ID, OVER_SLOT)
  ...
  it('does not arm off the slot', () => {
    expect(resolveSlotDropIntent(new Set(['home']), PAGE_ID, { x: 900, y: 700 })).toBeNull()
  })

  it('does not arm for a multi-selection — no answer for which view is active', () => {
    expect(resolveSlotDropIntent(new Set(['home', 'plain']), PAGE_ID, OVER_SLOT)).toBeNull()
  })

  it('does not arm for a non-frame — only frames are views', () => {
    expect(resolveSlotDropIntent(new Set(['plain']), PAGE_ID, OVER_SLOT)).toBeNull()
  })

  it('does not arm for the slot dragged over itself', () => {
    expect(resolveSlotDropIntent(new Set(['slot1']), PAGE_ID, OVER_SLOT)).toBeNull()
  })
  ...
  describe('coexistence with the reparent path', () => {
    it('the reparent path still ignores slots entirely', () => {
      ...
      const intent = resolveDropIntent(new Set(['home']), PAGE_ID, OVER_SLOT)
      expect(intent).toBeNull()
    })

    it('a real container still reparents, unaffected', () => {
      const intent = resolveDropIntent(new Set(['home']), PAGE_ID, { x: 900, y: 500 })
      expect(intent?.targetId).toBe('holder')
    })

    it('a frame over a real container never arms a slot drop', () => {
      expect(resolveSlotDropIntent(new Set(['home']), PAGE_ID, { x: 900, y: 500 })).toBeNull()
    })
  })
})

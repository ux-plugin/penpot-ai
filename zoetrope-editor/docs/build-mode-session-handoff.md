...
Branch: `worktree-interactions-phase-0`. Package: `zoetrope-editor/`.
...
- **Dev server:** `cd zoetrope-editor && pnpm dev --port 5188`. Run only **one** vite instance per
...
- **Scope:** work only in `render-wasm` + `zoetrope-editor`. The `zoetrope-editor` React UI (Build mode)
...
- [ ] commit with `git -C <worktree-abs-path>`, message `feat(zoetrope-editor): … (slice N)`,

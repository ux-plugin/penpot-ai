...
Status: **Phase 1 shipped** (commit `97fd7a039e`, merged to `develop`). **Phase 2 (tokens) — planned, tokens-only.** Target: `zoetrope-editor` only. `render-wasm` requires **no changes**.
...
**Boundary discipline:** every layer stays in `zoetrope-editor/src/lib`. `render-wasm` and the Penpot

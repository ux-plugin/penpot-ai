...
### 2.2 Frontend = the whole chat product (`zoetrope-editor`)
...
- The **main process** holds the key and makes the direct provider call (Node — no CORS
  gating); the renderer (`zoetrope-editor`) sends prompts over **IPC** and receives the stream.
...
### Phase 2 — Frontend chat on the AI SDK (`zoetrope-editor`)
...
### 8.2 Frontend (`zoetrope-editor`) — file-level

...
the Skia GL / WebGL2 / WASM renderer (`zoetrope-editor` → `render-wasm`) behaves identically
...
`zoetrope-editor`'s own Vite app, which already owns a bespoke build (Tailwind, an ES web
worker bundled via `?worker`, and WASM served from `public/wasm/`). Re-implementing that
inside electron-vite would be brittle.

- **Dev:** the window loads `zoetrope-editor`'s dev server (`http://localhost:5173`) — full
  app, HMR, WASM, worker, and fonts all work as-is.
- **Prod (offline):** `zoetrope-editor`'s `build:app` target produces a static bundle
...
pnpm --filter zoetrope-desktop-app dev         # starts zoetrope-editor dev + Electron
```

`dev` runs `zoetrope-editor`'s dev server and the Electron shell together (via
...
> The app needs the WASM glue in `zoetrope-editor/public/wasm`. Produce it once with
> `pnpm --filter zoetrope-editor build:wasm` (a Docker `render-wasm` build) — without it the
> dev server and the app build both fail to resolve `render-wasm.js`.

## Build / package (offline bundle)

```bash
pnpm --filter zoetrope-editor build:wasm              # one-time: WASM glue (Docker)
...
`build` runs `zoetrope-editor build:app` → static `dist-app`, builds the Electron
...
scripts/bundle-renderer.mjs  Copies zoetrope-editor/dist-app -> out/renderer
electron.vite.config.ts    Build config for main / preload / renderer
electron-builder.yml       Packaging config (asarUnpack **/*.wasm)
```

## How the shell maps to the zoetrope-editor integration landmines
...
1. **Custom protocol, not `file://`** — `src/main/index.ts` registers a privileged
   ...
   in dev the window leans on zoetrope-editor's own dev server.

...
/// selection.
pub fn node_quad(node: &Node) -> [Point; 4] {
...
mod tests {
    ...
    fn one_shape_reports_its_own_size_with_rotation_in_the_matrix() {
        ...
        let [w, h, cx, cy, a, b, c, d, e, f] = selection_rect(&[node_quad(&n)]);
    ...
    fn several_shapes_report_an_upright_hull() {
        ...
        let alone = selection_rect(&[node_quad(&rotated)]);
        assert!(alone[5].abs() > 0.1, "alone, the box is rotated");

        let together = selection_rect(&[node_quad(&rotated), node_quad(&other)]);
        assert_eq!(
    ...
    fn a_degenerate_shape_stays_finite() {
        ...
        let out = selection_rect(&[node_quad(&n)]);

...
const hexMap = [];
for (let i = 0; i < 256; i++) {
  hexMap[i] = (i + 256).toString(16).substring(1);
}
function bufferToHex(source, isUuid) {
  ...
  const spacer = "-";
  let i = 0;
  return hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + spacer + hexMap[src[i++]] + hexMap[src[i++]] + spacer + hexMap[src[i++]] + hexMap[src[i++]] + spacer + hexMap[src[i++]] + hexMap[src[i++]] + spacer + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]] + hexMap[src[i++]];
}
...
getBaseCodec(BASE62).encode;
const timeRef = 16409952e5;
const fill = (() => {
  const g = globalThis;
...
(function v8Factory() {
  ...
  const maxCs = 0x0000000000003fffn;
  let countCs = 0n;
  let lastRd = 0n;
  let lastCs = 0n;
  let lastTs = 0n;
  const baseMsb = 0x0000000000008000n;
  const baseLsb = 0x8000000000000000n;
  lastRd = nextLong() & 0xfffffffffffff0ffn;
  lastCs = nextLong() & maxCs;
  const create2 = function create22(ts, rd, cs) {
    const msb = baseMsb | rd & 0xffffffffffff0fffn;
    const lsb = baseLsb | ts << 14n & 0x3fffffffffffc000n | cs;
    setBigUint64(view, 0, msb, false);
    setBigUint64(view, 8, lsb, false);
    return bufferToHex(int8);
  };
  const factory = function v82() {
    while (true) {
      const ts = currentTimestamp(timeRef);
      if (ts - lastTs < 0) {
        lastRd = lastRd & 0x0000000000000f00n | nextLong() & 0xfffffffffffff0ffn;
        countCs = 0n;
        continue;
      }
      if (lastTs === ts) {
      ...
      return create2(ts, lastRd, lastCs);
    }
  };
  const fillBytes = (uuid) => {
    ...
    int8[9] = rest & 255;
    int8[10] = (rest = parseInt(uuid.slice(24, 36), 16)) / 1099511627776 & 255;
  ...
  const getBytes2 = (uuid) => {
    fillBytes(uuid);
    return Int8Array.from(int8);
  };
  const getUnsignedParts2 = (uuid) => {
    fillBytes(uuid);
  ...
  const fromUnsignedParts2 = (a, b, c, d) => {
    view.setUint32(0, a);
    view.setUint32(4, b);
    view.setUint32(8, c);
    view.setUint32(12, d);
  ...
  const setTag = (tag) => {
    ...
    if (t > 0x000000000000000fn) {
      throw new Error("illegal arguments: tag value should fit in 4bits");
    }
    lastRd = lastRd & 0xfffffffffffff0ffn | t << 8n & 0x0000000000000f00n;
  };
  factory.create = create2;
  factory.fromArray = fromArray;
  factory.fromPair = fromPair;
  factory.fromUnsignedParts = fromUnsignedParts2;
  factory.getBytes = getBytes2;
  factory.getHi = getHi2;
  factory.getLo = getLo2;
  factory.getUnsignedParts = getUnsignedParts2;

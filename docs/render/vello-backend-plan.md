...
Both modules expose the same logical surface so the host can load either interchangeably. The interop differs (Emscripten `Module._fn` vs wasm-bindgen), so each gets a thin TS adapter behind one `Renderer` interface in `zoetrope-editor`.
...
**Known naming wart:** `render-wasm` vs `render-vello` reads as though only one targets wasm. Both do; the honest pair is `render-skia`/`render-vello`. Renaming touches `render-wasm/build`, the docker devenv, `zoetrope-editor`'s `build:wasm` and the artifact name, so it is deferred rather than decided against. The same applies to `FocusRenderer`/`create_focus_renderer` in `render-vello`, which are leftovers from the superseded focus-mode framing (D3).
...
### D18 — Tiling: policy in `render-core`, store per backend; and why `vello_hybrid`, not classic Vello

**The driver is correctness, not only performance.** `vello_hybrid` applies filters in **viewport space**: `coarse.rs` clips a filter layer's content to the active viewport (`final_bbox = expanded_bbox.intersect(active_bbox)`) and rasterizes only into the viewport's wide-tiles. So an effect runs on a viewport-clipped *copy* of the shape — the viewport edge becomes a false shape edge, and blur/shadow vary with pan and zoom instead of being welded to the shape. `vello_cpu` gets this right via `source_expansion`/`source_padding` (the filter's *input* margin); the hybrid path never consults it. The σ-cap slices (Phase 5) bounded the symptom (effect reach ≤ 3σ ≤ ~255 device px) but did not remove the coupling. The structural fix is render-wasm's: render each device tile into its own **content+margin** buffer, run the effect inside the buffer, composite the center. Each tile buffer is its own little viewport with the margin baked in, so interior tiles are fully covered (no false edge) and edge tiles get a real, stable effect. Geometry mirrors render-wasm exactly — `TILE_SIZE = 512` content, `256` margin per side → a `1024²` buffer; `MAX_DEVICE_SIGMA = margins / 3 = 256 / 3 ≈ 85`, already the cap the shadow/blur slices ship, so an effect always fits the margin.

**Why `vello_hybrid`, not classic `vello`.** (a) The whole effects stack — the `custom-wgsl-filter` fork, inner shadow, the multi-pass graph, the filter atlas — is built on `sparse_strips` (`vello_common`/`vello_hybrid`); classic Vello has no equivalent custom-filter layer, so switching means rebuilding all of it. (b) Hybrid does **coarse rasterization on the CPU** (sparse strips) and fine on the GPU; that CPU seam is exactly where a retained per-tile cache lives — generate/cache strips per tile, reuse on pan, regenerate only dirty tiles. Classic Vello does coarse on the GPU in one whole-scene compute pass, built to redraw everything each frame, with no natural cache seam. (c) `vello_hybrid` ships a WebGL2 backend; classic Vello needs WebGPU compute. The one thing classic Vello wins — rendering a huge scene from scratch every frame — is the thing tile caching removes the need for.

**The seam: policy shared, store per backend.** "Tiling" is two things with different homes.

- **Policy → `render-core`** (GPU-free, host-tested): the tile grid, `TileKey { tile_x, tile_y, zoom_bucket, content_version }`, the dirty set, eviction/LRU, page-space→tile mapping, zoom-bucket selection, and the per-frame decision — three key-lists `render_set` / `evict_set` / `composite_set`. Pure key-math; no `HashMap<Key, Texture>`, no wgpu, no Skia. Sharing it is what makes both backends tile *identically*, so the two-URL comparison to Skia is honest, and it is testable against a fake store whose `Tile` is `()`.
- **Store → each backend, in full**, behind a small trait defined in `render-core`: `trait TileStore { type Tile; fn render_tile(key, scene, m) -> Tile; fn composite(&[(key, &Tile)], target); fn drop_tile(Tile); }`. The container is deliberately **not** in core. A generic `HashMap<Key, Texture>` forces one texture per tile keyed 1:1, which forecloses the two backend-exclusive wins that matter most:
  - **Vello:** a **texture-atlas + single-draw composite** (pack N tiles into one atlas/array texture, hold `(atlas_id, rect)`, composite a screenful in ~1 draw instead of N bind-group-switched draws — the cost is much higher on WebGL2, the backend we ship); and **coarse/strip caching** (cache the CPU strip output per tile — the exact cost that makes hybrid hybrid — a win with no analog on the Skia side, decisive for edits and zoom, where a *pixel* cache is invalid but geometry is stable).
  - **Skia:** `SkPicture` per tile (record draw ops once, replay at any zoom without re-projecting) plus Skia's own `GrResourceCache` (lean on built-in GPU residency instead of managing VRAM ourselves).
  Both are un-generic: the atlas breaks the 1:1 key→resource map; strip and picture caches are different data types with different lifecycles. So the *valuable* shared thing is the **policy engine**, not the container — core hands the store the three key-lists and the store owns the physical representation.

Tile keys are in **page space** (settled here, not left open): a pan then reuses tiles rather than invalidating them, the render-wasm behavior that gives the document its feel.
...
### Tiling
Superseded by D18. The earlier read — "a cross-frame tile cache is useless for globally dynamic scenes; pan/zoom invalidates every tile; effects need aprons" — was half right and missed the load-bearing half. With **page-space** keys a *pan* is pure tile reuse, not a full invalidation (only newly-exposed tiles render); only a *zoom* re-renders, and only the visible set (bounded even at 100×). And the apron is not a nuisance to bolt on: the per-tile margin is the very mechanism that makes effects correct, because `vello_hybrid` otherwise applies filters in viewport space (D18). So tiling is promoted from a perf nicety to a **correctness requirement**; the margin is `256` device px, cap `256 / 3 ≈ 85`, already shipped by the shadow/blur cap.
...
**Still ours.** Document → scene projection from `zoetrope-editor` and the wire format · the Skia-side adapter · the SkSL → WGSL effect ports · inner shadow and backdrop blur (Vello gaps) · dirty propagation and caching.
...
Feed `render-vello` a real document from `zoetrope-editor` rather than a hand-built model. **Wire format settled by D17**: reuse render-wasm's, with `Raw*Data` relocated into `render-core` so both backends parse identical bytes through identical definitions.

1. Move `Raw*Data` + `ToJs` from `render-wasm/src/wasm/**` into `render-core::abi`; render-wasm keeps its `From<Raw…> for shapes::Fill`.
2. Facade in `zoetrope-editor` presenting a `Module`-shaped object (`_name` methods, live `HEAPU8` getter) over a raw `WebAssembly.Instance`, so `api/*.ts` is untouched.
...
**The emscripten build is the gate that matters here** (D2: render-wasm must keep building for `wasm32-unknown-emscripten` against prebuilt Skia). Run it with `pnpm run build:wasm` from `zoetrope-editor` — it drives `render-wasm/build` inside `penpotapp/devenv`. Slice A passes it.
...
- **Slice 2 (done) — layer blur + drop shadows.** New `render_core::blur::radius_to_sigma` is the authority for the radius→sigma conversion (Skia's `0.577·r + 0.5`), pinned equal to render-wasm's own copy by `radius_to_sigma_agrees_with_render_core`. `Node` gained `blur: Option<f32>` (a layer-blur radius) and `shadows: Vec<Shadow>` (drop shadows only — `Shadow { color, blur, spread, offset }`); `digest_node` hashes both. render-wasm's `model_export` reads the layer blur and Drop shadows; render-vello implements the previously-stubbed `set_shape_blur` / `add_shape_shadow` / `clear_*`. **Dropped at projection on both sides** (like inner/outer strokes): background blur and inner shadows. Shadow **spread** is *carried* in the model (`Shadow.spread`, hashed) but not yet drawn this slice — the assumption at the time was that drawing it would need a dilate the fork lacks; a later slice found it does not (it is a CPU path outset — see the Shadow-spread entry under Phase 5).
...
- **The editor (done, three stages) — caret, selection, typing, IME.** Unlike everything else, the editor is *ephemeral interaction state*, not part of the model or its digest, so the cross-backend anchor is **unmoved** throughout. render-wasm hand-builds it on Skia's read-only paragraph API (~2800 lines across ~35 ABI entry points); render-vello wraps Parley's **`PlainEditor`**, which already does cursor movement, bidi-aware hit-testing, selection geometry and IME — so it is a *wrap + a bridge*, not a rewrite (~600 lines in a new `editor.rs` + `scene.rs`).
  ...
  **The plain-vs-rich boundary (closed by the next slice).** `PlainEditor` is single-style, so a focused multi-span run flattened to its first span's style *while editing*. That was the one real gap — see the rich-editing slice below.
  ...
  Verified: render-vello host 45; `zoetrope-editor` renderer suite 593 pass; anchor unmoved; three live pixel proofs on Vello in real Chrome — (1) click-drag produced a bidi-aware selection highlight with a caret; (2) an empty box typed to "Typed live on Vello", backspaced and retyped to "Typed live on Parley", `export_content` returning `[["Typed live on Parley"]]`; (3) an IME pre-edit "naïve café" composed inline with `get_cursor_rect` returning a caret rect.
...
- **Custom shaders — Tier 1 (done): a built-in effect library.** The design decision that shaped this: the fork's `FilterPrimitive::Custom { effect: u32, params: SmallVec<[f32; 8]>, expansion }` is **not** arbitrary user WGSL — `effect` indexes a `case Nu:` branch in a *compiled-in* `custom_effect` hook in `filters.wgsl`, and `params` is a flat uniform array. So "custom shader" on Vello today means a **curated, backend-authored effect library selected by index** (Tier 1), not runtime-authored shaders (Tier 2, a later fork extension: runtime `create_shader_module` + dynamic bind groups from reflected uniforms + a validation/denylist pass). This slice does Tier 1, Vello-first, on the effect the fork already ships — effect `0` = tint (`params = [r, g, b, amount]`).

  Neutral model gains `CustomEffect { id: u32, params: Vec<f32> }` on `Node`; `digest_node` hashes `id` + the params (input-hashing, like text and like the SkSL-side shader work — the neutral `(id, params)` is what lets a WGSL backend and an SkSL backend agree without a shared shader language). render-vello decodes it over a new `set_shape_custom_effect(effect)` ABI (params from the shared byte buffer) and applies it as the **outermost** filter layer wrapping the node's composited paint + children, via `push_filter_layer(FilterPrimitive::Custom{…})` — the same single-primitive mechanism as the blur/shadow slices. render-wasm has no Penpot author path for it yet, so `model_export` projects `None` (dropped at projection, like inner shadows); a Skia SkSL path for the same `id` is a later slice.

  Verified: render-core 64 (a new digest test — id, param values, and param count each move the hash), render-vello host 53 (a new ABI test decoding id + params from the buffer, and clearing), render-wasm compiles, wasm build clean; the cross-backend anchor moves to `2172659934` — both from the always-hashed effect slot and from a real tint effect added to the fixture text node, driving the `_set_shape_custom_effect` wire end to end through the published wasm. And the **pixel proof** — live on Vello in real Chrome: two identical `#1d4ed8` blue rects, the right one carrying effect `0` (tint toward orange, amount `0.7`), renders orange — the `custom_effect` WGSL branch running through the whole model → digest → wire → filter chain. Next: author a richer effect branch (glass/noise) — the first that edits `filters.wgsl` + rebuilds the fork's shaders — and then the Tier-2 runtime-shader extension.

- **Multi-pass filter graph (done, linear chain) — effects are Vello-only from here.** Direction locked with the user: shader effects are Vello-only (no Skia users), authored as a **multi-pass graph** of typed + raw nodes. A spike first de-risked the render side — the scary `unimplemented!("Multi-primitive filter graphs")` only rejects a *single* `Filter` with >1 primitive; **nesting filter layers** composes passes through the fork's render-graph intermediate textures and is fully implemented (`render_graph.rs`, `filter_offset_nested`). A sentinel-gated `offset → blur` nest, live on Vello in real Chrome (a rect visibly offset *and* blurred), proved it works through render-vello + WebGPU. Only **branching/merge** nodes (two inputs) still need the fork's multi-primitive graph — linear chains do not.

  Built on that: `render_core::model` replaces `CustomEffect` with `FilterGraph { nodes: Vec<FilterNode> }` where `FilterNode = Blur{sigma} | Offset{dx,dy} | Custom{effect,params}` (typed nodes for the engine to reason about; `Custom` is the raw-WGSL escape hatch). `Node.custom_effect` becomes `Node.filter_graph`; `digest_node` hashes the **ordered** node sequence (pass order changes the picture). render-vello's `set_shape_filter_graph` decodes a length-prefixed node stream from the shared buffer; `scene.rs` lowers the chain to nested filter layers — pushed in reverse so `nodes[0]` is innermost (applied first), popped by count. render-wasm projects `filter_graph: None` (Vello-only, no Skia draw). The one-pass `custom_effect` is now just a one-node `Custom` graph.

  Verified: render-core 64 (digest test — node kind, params, count *and order* each move the hash), render-vello host 53 (ABI test decoding a mixed blur·offset·custom chain, and clearing), render-wasm compiles, wasm build clean; cross-backend anchor moves to `1295223549` (the fixture text node now carries a real 3-node graph — blur→offset→tint — through the `_set_shape_filter_graph` wire). And the **pixel proof** — live on Vello in real Chrome: two `#1d4ed8` blue rects, the right one carrying a `tint(orange) → blur(σ6) → offset(40,40)` chain, renders orange, blurred, *and* shifted — all three passes composed via the decoded wire → nested filter layers. Next: the WGSL language provider + graph→WGSL emit for the shader focus stage (authoring), then branching/merge nodes (the remaining fork work).

- **Inner shadow (done) — the first effect that edits the fork.** With the graph in place, the effects lane was sequenced by dependency (inner shadow + shadow spread are standalone fork passes → first; soft masks needs an offscreen pass; the merge/branching graph is fork-executor work; backdrop blur rides on the caching layer → last). Inner shadow came first: it's `unimplemented!()` in the fork but a faithful mirror of `DropShadow` — **the same `offset → blur → composite` pipeline, only a new composite formula** (`inner = shadow_color · original.a · (1 − blurred.a)`, drawn over the shape). The fork edits are isolated to this worktree's `vello` submodule.

  Fork (5 files, each mirroring `DropShadow`): a `FilterPrimitive::InnerShadow` variant (`filter_expansion = ZERO` — it stays inside the shape, the key difference from drop shadow); `filter/inner_shadow.rs` (prepared struct); the `PreparedFilter` variant + arm; `vello_hybrid`'s `GpuInnerShadow` + `filter_type::INNER_SHADOW` + `pass_kind::COMPOSITE_INNER_SHADOW` + the `build_filter_passes` arm (offset→blur→composite_inner_shadow); and the `PASS_COMPOSITE_INNER_SHADOW` branch in `filters.wgsl`. Our stack: `FilterNode::InnerShadow{dx,dy,sigma,color}` (digest + a new tag on the wire), the ABI decode, and the `scene.rs` lowering.

  Verified: render-core 64, render-vello host 64, wasm build clean (the fork's WGSL recompiles — proof the shader parses); cross-backend anchor moves to `3280782290` (the fixture graph now carries an inner-shadow node through the wire); render-wasm untouched (no emscripten gate). And the **pixel proof** — live on Vello in real Chrome: two white rects, the right one with an inner shadow (`dx/dy 14, σ9, ~80% black`) renders a soft dark shadow *inside* its top-left edges (recessed look), staying within the shape — correct on the first render, validating the composite formula and the zero-expansion choice.

- **Shadow spread (done) — and it needed no fork edit, correcting a standing assumption.** The plan had spread down as "a dilate the fork lacks" (a GPU morphology pass), sequenced next to inner shadow as fork work. But render-vello's drop shadow is *not* the fork's alpha-morphology `DropShadow`; it is a **CPU-filled path** — `draw_drop_shadows` fills `outline(node)` in the shadow colour inside a `GaussianBlur` layer. So spread is a **path outset**, not an alpha dilation: grow the silhouette by `spread` before the blur. No fork edit, no new WGSL, no dilate pass.

  New `render_vello::geometry` module (host-testable, so lifted out of the `wasm32`-gated `scene`): `spread_outline(node, spread)` grows the analytic shapes exactly and fills once — a rect and its corner radii, or an oval, each inflate by `spread` (matching Skia's `dilate((spread, spread))`); a general vector path grows by its Minkowski sum with a disk (a round-join `2·spread` stroke unioned with the interior under non-zero winding, so a semi-transparent shadow does not darken where band meets fill). Only positive spread grows, mirroring render-wasm (`if spread > 0`). The model already carried `spread` (hashed since Slice 2, decoded by the ABI, projected by render-wasm), so **nothing changed in render-core, the wire, or render-wasm** — the anchor does not move — and this slice is purely the Vello *apply*.

  Verified: render-core 64, render-vello host **57** (4 new `geometry` unit tests: rect/circle inflate exactly, rounded corners grow with the rect, a triangle path grows outward on every side); wasm build clean. **Pixel proof** — live on Vello in real Chrome: two identical blue rects with the same `σ12`, `(18,18)` semi-transparent-black shadow, the left at `spread 0`, the right at `spread 26`. The left shadow hugs the shape; the right balloons into a much larger, denser halo spilling well past every edge — exactly the dilation, on the first render.

  **A pre-existing bug the spread proof surfaced, fixed here.** Zooming in made a shadow's *blur* stop growing — it collapsed to a fixed device-pixel edge that read as "the shadow narrows to fit the viewport". Cause: `push_layer` captures the transform current at push time and scales the filter's sigma/expansion by it, but `draw_drop_shadows` pushed the blur layer *before* setting the shape's (zoom-scaled) transform, so the blur was scaled by a stale, un-zoomed transform. Fixed by setting the transform first (the order the layer-blur and filter-graph paths already used). The blur's softness now tracks the zoom like the rest of the shape. `spread` was never affected — it is page-space geometry drawn under the zoomed matrix.

  **A second, deeper shadow bug the fix above then exposed — and its resolution is parity with Skia.** With the blur correctly tracking zoom, zooming *far* in made the shadow **fade to nothing**. Isolated with a controlled single-frame sweep (fixed view, only the blur magnitude varied): the fork's decimated Gaussian (a downscale→blur→upscale pyramid) **loses energy once it stacks many levels**, so a large device sigma yields a washed-out shadow far lighter than the maths predicts. The cause of the fade under zoom is that render-vello let the device sigma grow without bound. **render-wasm never does:** it caps the shadow/layer blur at `sigma_device = min(radius_to_sigma(blur)·scale, margins.width / 3)` — the tile margin is `TILE_SIZE(512)·2/4 = 256` device px, so the ceiling is `256/3 ≈ 85` px (`get_drop_shadow_filter_capped`). The Skia backend refuses to build a bigger blur because the kernel must fit the tile margin. So the fix is to mirror that cap, not to re-engineer the fork's blur: `geometry::cap_sigma_to_device` clamps the user-space sigma to `MAX_DEVICE_SIGMA / scale` (scale = the matrix's mean axis norm, the same factor the fork's `transform_blur_params` applies), bounding the device sigma to `256/3` for both the drop shadow and the layer blur. Past the cap the shadow stops getting softer but stays dark and well-formed — identical to the Skia renderer. Verified: render-vello host **60** (3 new `cap_sigma_to_device` tests — pass-through at unit scale, clamp-to-ceiling at 100×, zero-scale guard); wasm build clean; and a **pixel proof** — the exact `size 400, radius 200` frame that was a faint grey wash now renders a dark, crisp shadow, and a fixed shape zoomed to 14× keeps a solid dark edge instead of vanishing.

  **A refinement over render-wasm's blur-only cap: also cap the offset.** Clamping only the blur (as render-wasm does) freezes the softness at the ceiling while the shadow's *offset* keeps growing with zoom, so the offset drifts away from the frozen blur and the shadow's shape changes as you zoom — very visible on an inner shadow, whose dark-band thickness *is* the offset (the band kept thickening past the cap). `geometry::cap_shadow_blur` returns the factor the blur was clamped by, and the caller scales the offset by the same factor, so once the blur plateaus the offset plateaus with it and the whole shadow keeps its shape (drop and inner alike). render-wasm's own comment justifies the blur-only cap by assuming the shadow is "a uniform wash" past the cap — true only when the offset is small next to the blur, which an inner shadow violates. Verified: render-vello host 61 (a `cap_shadow_blur` ratio test); pixel proof — an inner shadow (offset 14, blur 10) at 18× is a clean proportional recess rather than a thick offset-dominated band.

- **Inner shadow from the UI (done) — wiring the fork primitive to the shadow list.** The fork's `InnerShadow` (built earlier) was only reachable through the *filter-graph* wire, but the app authors an inner shadow as an ordinary entry in the shape's **shadow list** (`_add_shape_shadow` with `RawShadowStyle::Inner`). That wire *dropped* inner shadows on both sides — so an inner shadow set in the UI rendered nothing on Vello. Fixed by carrying inner shadows through the shadow path end to end: `render_core::Shadow` gains an `inset: bool` (digest hashes it); the ABI and render-wasm's `model_export` stop dropping inner shadows and tag them `inset` (only hidden ones drop now); and `scene.rs` renders them — drop shadows behind the shape (as before), inner shadows *inside* it via `push_inner_shadows`, which wraps the shape's own `paint_self` in one `FilterPrimitive::InnerShadow` layer per inset shadow (an inner shadow is a property of the shape's pixels, so it must enclose the fill, not sit behind it). Blur is capped and zoom-scaled exactly like the drop shadow; `spread` is ignored for inner shadows, matching render-wasm.

  Verified: render-core 64, render-vello host 60, render-wasm `cargo check` clean; the cross-backend anchor moves to `3919334440` (the `inset` field now crosses the wire and is hashed) with the invariance tests still green. **Pixel proof** — two white rects driven through the real `_add_shape_shadow` wire the UI uses: the left with a drop shadow (style 0) casts it behind and offset; the right with an inner shadow (style 1) darkens *inside* its top-left edges, the recessed look, correct on the first render.

  Next by the dependency order: soft masks (the offscreen render-to-texture pass).

### Phase 6 — tiled rendering + caching + animator

**Promoted (D18):** the tiled render path below is no longer only a perf phase — it is the structural fix for effect viewport-coupling, so it lands ahead of the animator.

**6a — tiled render + cache (the D18 seam).** `render-core` gains the GPU-free policy: `TileKey`, the grid, dirty set, eviction, page-space mapping, zoom buckets, the per-frame `render_set` / `evict_set` / `composite_set`, and `trait TileStore` — all host-tested against a fake store. render-vello gains a `VelloTileStore` implementing it. Slices:
  1. **Trivial store, no persistence (done).** New `render_core::tiling` (host-tested, GPU-free): `TILE_SIZE 512` / `TILE_MARGIN 256` / `TILE_BUFFER 1024`, `TileKey{tile_x,tile_y,zoom_bucket}`, `view_scale`, `zoom_bucket`, `visible_tiles`, `tile_device_origin`, `tile_render_transform`, and the `TileStore` trait — 7 tests (grid coverage, integer pan-shift, sub-tile pan exposing a column, render-transform centring, degenerate views). render-vello's `tiles.rs` renders every visible tile into its own `1024²` `Scene` (so the fork's `active_bbox` is the tile+margin, not the screen) and blits the centre `512` onto the swapchain.

     **Two bring-up findings.** (a) *The composite must be a GPU copy, not a Vello draw.* The first cut composited the centres with the fork's own `draw_texture_rects` (each tile bound as an external `TextureId`); binding ~15 spatially-adjacent tile textures in one pass tripped the fork's external-texture *run* scheduler, so some tiles fell back to the black placeholder texture — a parity of black vertical bands. `copy_texture_to_texture` (tile `RENDER_ATTACHMENT|COPY_SRC` → surface `RENDER_ATTACHMENT|COPY_DST`) sidesteps that scheduler, is pixel-exact (1:1, no sampling), and needs no shader. It requires WebGPU (copy-to-swapchain); the WebGL2 fallback would need the draw path and is deferred. (b) *Tiles must be placed by the **effective** view, not the harness `root`.* `NeutralModelScene::render` composes `root * viewport` internally, so when the host drives, `root` is identity and all the pan/zoom lives in the viewport; the tile geometry has to use `effective_view = root * viewport` (new `abi::effective_view`, mirroring the draw path incl. the live-vs-fallback branch) and pass each tile `offset * root` so the scene reconstructs `offset * full_view`. Also wired `console_log` into the library path (`create_focus_renderer`) at Warn — it only existed in the mock-host `main.rs`, so frame-skip warnings had been silently dropped.

     Verified: render-core 71, render-vello 61 host tests; wasm builds clean. **Pixel proof** — live on Vello in real Chrome (WebGPU), the host document tiled: white background fills, a layer-blurred rect and a drop-shadowed blue rect render with no black bands and seamless tile joins (shape edges, not tile lines — confirmed against the Skia render of the same document), and zooming to 169% keeps the drop shadow welded to the blue rect and scaling with it — the effect is now shape-relative, not viewport-relative, which was the whole point.

     **Correction (post-proof) — effects are gated off in the tile store; slice 1's scope is plain content.** The proof above was optimistic: running effects *per tile* is not translation-invariant under the fork's decimated Gaussian (a multi-scale 2×-pyramid blur whose halo/phase shifts with the tile offset), so blur/shadow **seam at tile joins** and flicker under pan/zoom. Effects are therefore gated off in `tiles.rs` (thread-local `EFFECTS_ENABLED`, off while tiling) and deferred to their own world-anchored `extrect`-anchored surfaces composited into tiles — the genuine "larger issue", not a per-tile pass. Slice 1's real deliverable is **plain content tiled correctly**: fills + strokes, seamless, no flicker.

     **And a corruption fix the plain-content pass surfaced.** Multi-tile frames showed stroke "teeth" (top-edge notches). Root cause, isolated with a deterministic `tiling-harness.html` (whole-scene clean, single-tile clean, multi-tile teeth → it was the *number of render passes per submit*, not the transform): the fork's `Renderer.render()` uploads each frame's strip/alpha/paint data into **shared GPU buffers** via `programs.prepare` (queue writes), so batching N tile renders onto one `encoder`+`submit` let each tile's upload overwrite the previous before the GPU ran its draws — every tile sampled the *last* tile's buffers. **Fix: one `queue.submit()` per tile render**, pairing each render's uploads with its own draws (perf cost: N submits/frame, to be reduced later). The harness and a `set_tiling_bypass` diagnostic toggle are kept as bring-up aids.
  2. **Persistent cache, page-space keys (done) — a pan is pure tile reuse (the document feel).** `VelloTileStore` keys rendered buffers by `TileKey` in a `HashMap` and reuses them across frames; only newly-exposed tiles rasterise, the rest composite straight from cache. Sound because a tile's content is *pan-invariant* — `tile_render_transform` maps a page point to `scale·p + (margin − tile·512)`, so the view's pan `(e,f)` cancels and a given `(tile_x,tile_y)` renders identical pixels at a fixed scale regardless of scroll. The composite is 1:1 at the exact current scale, so any **scale change** (even within one `zoom_bucket`) drops the whole cache and re-renders — zoom reuse is slice 3. A pure pan keeps `view_scale` bit-identical (it reads only the linear part), so the guard reuses on pan and never falsely on zoom. Eviction is LRU over a `MAX_CACHED_TILES = 48` budget of *non-visible* tiles (visible tiles always retained); evicted textures return to a recycle pool rather than being dropped, so a zoom — invalidate-then-re-render a screen — reuses the freed textures instead of reallocating. A new `last_tile_stats` export (packed `rendered<<16 | reused`) makes reuse machine-checkable.

     Verified: render-core 7 tiling + render-vello 61 host tests green; wasm builds clean. **Quantitative proof** — live on Vello (WebGPU) in the tiling harness, reading `_last_tile_stats` per frame: a repeated view is `0 rendered / 4 reused`; a +200px pan is `2 rendered / 4 reused` (only the newly-exposed column re-rasterises); a zoom to 2× is `6 rendered / 0 reused` (scale change correctly invalidates). **Pixel proof** — a sub-tile pan producing a 9-tile frame of `5 freshly-rendered + 4 cached` composites with solid, continuous strokes and no seam at any fresh/cached join (`proofs/cache-mixed-frame.png`).
  3. **Invalidation + zoom buckets** — an edit dirties only the tiles it touches; a zoom re-renders only the visible set at device resolution (bounded and crisp even when the shape is 100× the screen).
  4. **Retire the interim whole-scene σ-cap hacks** where the per-tile margin now covers them, and layer the Vello-only wins on as store-internal upgrades (texture-atlas single-draw composite; coarse/strip caching) — no core change.

**6a-effects — effects through one surface scheduler.** *(Supersedes the earlier "world-anchored `extrect`-anchored per-shape surfaces, composited once" deferral and the `EFFECTS_ENABLED` gate: those were the right instinct — produce an effect on a surface sized to its extent, not per-tile — but framed as a bolt-on shadow "lane." That was special-casing. The faithful design is the one render-wasm already ships: a single scheduled pipeline over a surface pool.)*

*Why one pipeline, not two lanes.* In render-wasm's SSA a shape and its shadows are **one** `Paint` step into **one** surface — [step.rs](render-wasm/src/tile_grid/ssa/step.rs): *"one `Paint` step covers a shape's full body (fills + strokes + drop/inner/text shadows)."* There is no separate shadow producer; there is one schedule where **everything is a surface** — a tile is a surface, an isolated shape body is a surface, a gather's backdrop is a surface — and steps paint into and composite among them, interleaved in z-order by one dispatcher over one allocator. Vello cannot inline-composite a previously-rendered GPU texture the way SkCanvas can `drawImage` a snapshot mid-stream, so a "composite" is a separate blend pass rather than an inline draw — but that is *two mechanisms in one sink*, not two pipelines.

*The model.*
- **Surfaces** are the currency: GPU textures from a pool. Sized to what each needs — a tile is `TILE_BUFFER²`; an isolated shape body is its **`extrect`** (selrect grown by the spread: shadow offset + blur reach + `spread`); a gather backdrop is its **sample rect** (`GatherKind::extent_world`). Sizing to the extent — not to a fixed tile margin — *is* the seam fix (the margin ignored the shadow offset, so it clipped under zoom).
- **Steps** (minimal set, mirroring the SSA `Step` enum): `Paint` (draw a shape's whole body — fills + strokes + drop/inner shadows — into a surface: straight into a tile when the shape has no effects/isolation, else into an `extrect`-sized intermediate); `Composite` (blend one surface into another with opacity/blend/clip — this is [`blend.rs`](render-vello/src/blend.rs)); and the gather pair `ComposeBackdrop` (snapshot the frame-so-far across the sample rect into a backdrop surface) + `PaintGather` (run Glass/BackgroundBlur reading that backdrop).
- **Two effect *kinds*, scheduled oppositely** — first-class in the IR, not discovered later:
  - **Spread** (drop/inner shadow, layer blur, glow): `Paint::reads()` is empty — inputs are shape data, the pixels spread *outward*. Paint into an `extrect`-sized surface, composite into every tile the extrect overlaps.
  - **Gather** (background blur, glass): reads *inward* from the backdrop beneath. `ComposeBackdrop`→`PaintGather`. Its backdrop must be sized to the full sample rect for the same reason spread surfaces are — the fork's own note warns the tile-sized backdrop "crops off → blur fades on zoom-in."
- **Dependency graph** orders producer→consumer. Flat spread reduces to "paint the surfaces, composite in z-order"; gather's backdrop-read **forces true z-order interleaving** (so the "shadows-then-shapes" shortcut is dropped — it only ever held because spread does not read the backdrop).

*Homes (D18 / the scheduler-in-core decision).* **render-core (neutral):** the schedule IR (`SurfaceRef`, `Step` incl. effect-kind), the schedule builder (z-order walk → steps), the dependency graph, the surface **sizing** policy, and tile **assignment** (`tiles_overlapping_page_rect`, done). Triangulated against `render-wasm/src/tile_grid/ssa/{step,dep_graph,schedule_builder}` — reference, not blind port, per the "extract when the second consumer exists" rule. **render-vello (GPU sink):** the surface **pool**, executing `Paint` via a `vello_hybrid` render-into-surface (reusing `draw_node`/`draw_drop_shadows` unchanged), `Composite`/backdrop-snapshot via `blend.rs`, and the gather effect pass. GPU-only — **no `vello_cpu`**; no fork edit for spread (gather's effect pass may need a WGSL primitive, which lands on `penpot/effect-tile-surfaces`).

*Already built toward this:* `render_core::tiling::tiles_overlapping_page_rect` (+4 host tests); `render_vello::blend` — the SrcOver blit `Composite` mechanism (compiles against the fork's patched wgpu). Reused as-is: the whole `draw_node` path.

*Phasing.*
1. **Unified skeleton, spread-only, drop shadow first (this slice).** Surface pool + `Paint`(direct | to-`extrect`-surface) + `Composite`, z-ordered; background becomes the composite clear colour; content tiles gate the background out (`DRAW_BACKGROUND` flag) so shapes composite over shadows. A drop-shadowed shape paints its whole body into one `extrect` surface, composited into the overlapped tiles. **Verify:** numeric pixel diff vs the whole-scene bypass → ~0 at zoom 1 / 1.7 / high — the seam gone.
2. **Remaining spread effects** (layer blur, inner shadow, glow) — same `Paint`-into-surface path, no new step kinds.
3. **Gather effects** (background blur, glass) — add `ComposeBackdrop`/`PaintGather` + the sink's backdrop-read; true z-order interleaving.
4. **Lift the neutral schedule IR into render-core** properly (triangulated against `dep_graph`), grow liveness / allocation / eviction, and unify the effect-surface pool with the tile cache.

*Verification bar throughout:* the numeric pixel diff vs the whole-scene bypass at multiple zooms — the methodology that caught the seam in the first place (a "looks seamless" eyeball is not accepted).

**6b — node dirty propagation (animator).** Rive-style dependency-ordered dirty propagation (D8), following Masonry's shape: a retained recorded scene per node, re-recorded only when a dirty flag is set. Composes with 6a — a dirtied node dirties the tiles it touches.

**Benchmark** against Skia using present-cadence on real files — **including a pan and a zoom**, the cases that dirty everything. This phase is also the gate on Phase 7: per D16, approach A cannot be judged before dirty gating exists and that measurement has been taken.

...
- **Inner shadow (done) — the first effect that edits the fork.** With the graph in place, the effects lane was sequenced by dependency (inner shadow + shadow spread are standalone fork passes → first; soft masks needs an offscreen pass; the merge/branching graph is fork-executor work; backdrop blur rides on the caching layer → last). Inner shadow came first: it's `unimplemented!()` in the fork but a faithful mirror of `DropShadow` — **the same `offset → blur → composite` pipeline, only a new composite formula** (`inner = shadow_color · original.a · (1 − blurred.a)`, drawn over the shape). The fork edits are isolated to this worktree's `vello` submodule.
  ...
  Verified: render-core 64, render-vello host 64, wasm build clean (the fork's WGSL recompiles — proof the shader parses); cross-backend anchor moves to `3280782290` (the fixture graph now carries an inner-shadow node through the wire); render-wasm untouched (no emscripten gate). And the **pixel proof** — live on Vello in real Chrome: two white rects, the right one with an inner shadow (`dx/dy 14, σ9, ~80% black`) renders a soft dark shadow *inside* its top-left edges (recessed look), staying within the shape — correct on the first render, validating the composite formula and the zero-expansion choice. Next effect by the dependency order: shadow spread (a dilate pass).

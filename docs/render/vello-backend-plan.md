...
- **Slice D (done)** — host wiring. `api/*.ts` drives render-vello unmodified, checked by execution: `vello-instance.ts` instantiates the real wasm in Node with all ~600 bindgen glue imports stubbed as throwers (the ABI path calls none of them), and the real `node-factory` → `orchestration::setObject` → `viewport::setViewBox` chain runs against it. In the app, `?renderer=vello` loads the Vello artifact, acquires a WebGPU adapter and syncs shapes into its scene.
  ...
  **Modifiers are the gap this exposed.** `get_selection_rect` earns its place in the renderer at all because render-wasm's `shapes_pool::get` returns `shape.transformed(modifiers, …)`, so the query reports live drag/scale bounds that the committed document — and therefore the worker's index — does not have. render-vello implements no modifiers (`set_modifiers`, `propagate_modifiers`, `set_absolute_modifiers`, `set_structure_modifiers`, `clean_modifiers` all hit the stub), so its answer is committed geometry, and a drag shows no preview at all. That is the next functional gap, ahead of anything cosmetic.

...
pub(crate) const DISPLACEMENT_FORMAT: wgpu::TextureFormat = wgpu::TextureFormat::Rgba16Float;

pub(crate) struct GlassPipeline {
...
impl GlassPipeline {
    ...
    pub(crate) fn displacement(&self, device: &wgpu::Device, encoder: &mut wgpu::CommandEncoder, target: &wgpu::TextureView, u: &[f32; 20]) {
    ...
    pub(crate) fn refraction(&self, device: &wgpu::Device, encoder: &mut wgpu::CommandEncoder, target: &wgpu::TextureView, backdrop: &wgpu::TextureView, disp: &wgpu::TextureView, u: &[f32; 4]) {
        let uniform = Self::uniform(device, u);
        let bind = device.create_bind_group(&wgpu::BindGroupDescriptor {
    ...
    pub(crate) fn composite(&self, device: &wgpu::Device, encoder: &mut wgpu::CommandEncoder, target: &wgpu::TextureView, blurred: &wgpu::TextureView, original: &wgpu::TextureView, disp: &wgpu::TextureView, u: &[f32; 8]) {
        let uniform = Self::uniform(device, u);
        let bind = device.create_bind_group(&wgpu::BindGroupDescriptor {

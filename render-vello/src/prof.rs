...
pub(crate) fn add_build(ms: f64) {
    BUILD.with(|c| c.set(c.get() + ms));
}
pub(crate) fn add_scene(ms: f64) {
    SCENE.with(|c| c.set(c.get() + ms));
}
pub(crate) fn add_render(ms: f64) {
    RENDER.with(|c| c.set(c.get() + ms));
}
pub(crate) fn add_submit(ms: f64) {
    SUBMIT.with(|c| c.set(c.get() + ms));
}
pub(crate) fn add_tex(ms: f64) {
    TEX.with(|c| c.set(c.get() + ms));
    TEXN.with(|c| c.set(c.get() + 1));
}
pub(crate) fn inc_step() {
...
pub(crate) fn read(which: u32) -> f64 {
    match which {

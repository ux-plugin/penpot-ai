 ...
 * 1. GET /auth/auth0/login → { readTokenJwt, loginUrl }
 * 2. Open Auth0 hosted login in a popup; user picks GitHub/Figma/etc.
 * 3. GET /auth/auth0/access-token (long-poll) → Auth0 tokens once the popup completes.
...
export async function getAuth0Login(signal?: AbortSignal): Promise<Auth0LoginAuthData> {
  ...
  const { readTokenJwt, loginUrl }: LoginInitResponse = await initResponse.json();

  window.open(loginUrl, "_blank");

  const tokensResponse = await fetch(`${baseUrl}/auth/auth0/access-token`, {
    method: "GET",
    headers: {
      Accept: "application/json",
      Authorization: `Bearer ${readTokenJwt}`,
    },
    signal,
  });

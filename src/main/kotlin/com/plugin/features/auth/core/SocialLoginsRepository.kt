...
class SocialLoginsRepository(private val database: R2dbcDatabase) {
    ...
    /** Find by ID */
    suspend fun findById(id: String): SocialLoginEntity? =
    ...
    suspend fun findByUserIdAndId(userId: String, id: String): SocialLoginEntity? =
        suspendTransaction(database) {
            SocialLoginsTable.selectAll()
    ...
    suspend fun save(entity: SocialLoginEntity): SocialLoginEntity =
        suspendTransaction(database) {
            val existing =
                SocialLoginsTable.selectAll()
    ...
    suspend fun deleteById(id: String): Int =
        suspendTransaction(database) { SocialLoginsTable.deleteWhere { SocialLoginsTable.id eq id } }

...
import { createPortUpdatesConnection, PortUpdatesData } from '@/api/backend/user/portUpdates';
  ...
  connect: async () => {
    ...
    try {
      const newConnection = createPortUpdatesConnection(
        ...
        // onError callback
        // TODO this is not being called, but the error is being logged to the console
        (errorMessage) => {
